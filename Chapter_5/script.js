...
if (boss.state !== 'attacking' && boss.state !== 'jump' && boss.state !== 'dead') {
        boss.state = 'hurt';
        boss.currentFrame = 0;
        boss.frameTimer = 0;
    }
...
if (boss.state !== 'attacking' && boss.state !== 'jump' && boss.state !== 'hurt') {
            boss.state = 'hurt';
            boss.currentFrame = 0;
            boss.frameTimer = 0;
        }
...
if ((p1Ativou || p2Ativou) && !boss) {
    ...
    boss = {
        ...
        // Frames Específicos (Conforme seu pedido)
        idleFrames: 5, walkFrames: 8, runFrames: 8, 
        jumpFrames: 7, hurtFrames: 1, deadFrames: 8, protectFrames: 2,
        attackFrames: 5,

        // Imagens
        imgIdle: new Image(), imgWalk: new Image(), imgRun: new Image(),
        imgJump: new Image(), imgHurt: new Image(), imgDead: new Image(),
        imgProtect: new Image(),
        
        // Imagens dos Ataques (Carregamos todas para trocar dinamicamente)
        imgAttack1: new Image(), imgAttack2: new Image(), imgAttack3: new Image(),
        imgAttack: new Image(), // Esta é a imagem "ativa" que o draw() usa
        
        // IA e Falas
        fala: "", falaTimer: 0, viuPlayer: false,
        isProtecting: false, protectTimer: 0, protectCurrentFrame: 0, protectFrameTimer: 0,
    };

    const folder = '../assets/enemies/Converted_Vampire';
    
    boss.imgIdle.src   = `${folder}/Idle.png`;
    boss.imgWalk.src   = `${folder}/Walk.png`;
    boss.imgRun.src    = `${folder}/Run.png`;
    boss.imgJump.src   = `${folder}/Jump.png`;
    boss.imgHurt.src   = `${folder}/Hurt.png`;
    boss.imgDead.src   = `${folder}/Dead.png`;
    boss.imgProtect.src= `${folder}/Protect.png`;

    // Carrega os 3 ataques
    boss.imgAttack1.src = `${folder}/Attack_1.png`;
    boss.imgAttack2.src = `${folder}/Attack_2.png`;
    boss.imgAttack3.src = `${folder}/Attack_3.png`;
    
    // Define o ataque padrão inicial
    boss.imgAttack = boss.imgAttack1;
    
    // Ajuste de hitbox visual se necessário
    console.log("O Vampiro despertou!");
}
...
function updateBossLogic() {
    ...
    // Distância simples (centro a centro fica mais preciso para IA de perseguição)
    const centroBoss = boss.x + boss.width / 2;
    const centroAlvo = alvo.x + alvo.width / 2;
    const dist = Math.abs(centroAlvo - centroBoss);
    ...
    if (boss.state === 'dead') {
        ...
        if (boss.frameTimer >= boss.frameInterval) {
            boss.frameTimer = 0;
            if (boss.currentFrame < boss.deadFrames - 1) boss.currentFrame++;
        }
        return;
    }

    // --- 3. FÍSICA E COOLDOWNS ---
    ...
    // --- 4. SUPER ARMOR ---
    boss.canBeStunned = (boss.state === 'idle' || boss.state === 'walking' || boss.state === 'running');

    // --- 5. ANIMAÇÃO ---
    boss.frameTimer++;
    let speedMod = boss.isPhase2 ? 0.8 : 1.0; 
    
    if (boss.frameTimer >= boss.frameInterval * speedMod) {
        boss.frameTimer = 0;
        let maxFrames = boss.idleFrames;

        if (boss.state === 'walking') maxFrames = boss.walkFrames;
        else if (boss.state === 'running') maxFrames = boss.runFrames;
        else if (boss.state === 'attacking') maxFrames = boss.attackFrames;
        else if (boss.state === 'hurt') maxFrames = boss.hurtFrames;
        else if (boss.state === 'jump') maxFrames = boss.jumpFrames;
        ...
        if (boss.state === 'attacking') {
            let hitFrame = 2;
            if (boss.imgAttack === boss.imgAttack2) hitFrame = 1; 
            
            // Só calcula colisão no frame exato do impacto
            if (boss.currentFrame === hitFrame) {
                
                // Criação da Hitbox do Ataque (Retângulo real, não só distância)
                // Se olhar p/ direita, a hitbox começa no meio do boss e vai pra frente
                // Se olhar p/ esquerda, começa antes do boss
                let alcanceAtaque = boss.attackRange + 30; // Margem de erro
                let attackX, attackW;

                if (boss.facing === 'right') {
                    attackX = boss.x + (boss.width * 0.4); 
                    attackW = alcanceAtaque;
                } else {
                    attackX = boss.x + (boss.width * 0.6) - alcanceAtaque; 
                    attackW = alcanceAtaque;
                }

                // Verifica colisão Retângulo x Retângulo com o Alvo
                if (
                    attackX < alvo.x + alvo.width &&
                    attackX + attackW > alvo.x &&
                    boss.y < alvo.y + alvo.height &&
                    boss.y + boss.height > alvo.y
                ) {
                    // VERIFICA SE O ALVO BLOQUEOU
                    if (alvo.isProtecting) {
                         // Som de metal batendo ou faisca
                         console.log("Boss atingiu defesa!");
                         // Opcional: Empurra mesmo defendendo
                         alvo.x += (boss.facing === 'right' ? 10 : -10);
                    } else {
                        // Dano Real
let finalDamage = boss.damage + (boss.isPhase2 ? 1 : 0);
alvo.hp -= finalDamage;
...
if (alvo.hp <= 0) {
    ...
    // Knockback
    alvo.velX = (boss.facing === 'left' ? -20 : 20);
    triggerShake(10);
}
        ...
        if (boss.currentFrame >= maxFrames) {
            ...
            if (boss.state === 'jump') {
                // Teleporte: Aparece atrás do alvo
                if (boss.facing === 'left') boss.x = alvo.x + 80;
                else boss.x = alvo.x - 80;
                ...
                boss.attackFrames = 3;
                boss.frameInterval = 6;
                bossDiz("Lento demais!");
            }
            else if (boss.state === 'attacking' || boss.state === 'hurt') {
                if (boss.isPhase2 && Math.random() < 0.5 && dist < 120 && boss.state === 'attacking') {
                     boss.state = 'attacking';
                    ...
                    boss.state = 'idle';
                    boss.attackCooldown = boss.isPhase2 ? 20 : 50; 
                }
            }
        }
    }
   ...
   npcs.forEach(n => {
    ...
    if (gameState === 'playing') {
        ...
        if (boss && boss.hp > 0) {
            ...
            ctx.fillText("VAMPIRE LORD", canvas.width/2, 35);
        }
    }
    ...
    if (screen) {
        ...
        // CASO B: VITÓRIA (Boss morreu)
            ...
            if (screen.style.display !== 'flex') {
                ...
                if (title) title.innerHTML = "Você derrubou <br> VAMPIRE LORD";
                if (subtitle) subtitle.innerHTML = "Finalmente, a energia maligna começa a se dissipar!";
